interface RecommendedSearchTermDeleteModalProps {
  isOpen: boolean
  onClose: () => void
  onConfirm: () => void
  term: string
...
const RecommendedSearchTermDeleteModal = ({
  isOpen,
  onClose,
  onConfirm,
  term,
  isLoading = false,
}: RecommendedSearchTermDeleteModalProps) => {
  const handleClose = () => {
    if (!isLoading) {
      onClose()
    }
...
export default RecommendedSearchTermDeleteModal
